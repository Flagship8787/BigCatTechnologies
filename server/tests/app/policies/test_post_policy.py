...
def make_policy(sub: str, scope: str) -> PostPolicy:
    token = SessionToken(sub=sub, scope=scope)
...
class TestPostPolicyScopePublish:
    ...
    @pytest.mark.asyncio
    async def test_admin_scope_returns_select_post(self, db_session: AsyncSession):
    ...
    @pytest.mark.asyncio
    async def test_posts_admin_scope_returns_select_post(self, db_session: AsyncSession):
        post = await create_post(db_session)
    ...
    @pytest.mark.asyncio
    async def test_own_scope_returns_own_posts(self, db_session: AsyncSession):
    ...
    @pytest.mark.asyncio
    async def test_own_scope_excludes_other_users_posts(self, db_session: AsyncSession):
    ...
    @pytest.mark.asyncio
    async def test_no_scope_raises_not_authorized(self, db_session: AsyncSession):
        policy = make_policy("auth0|user1", "")
        with pytest.raises(NotAuthorized):
            policy.scope("publish")

    @pytest.mark.asyncio
    async def test_unrecognized_scope_raises_not_authorized(self, db_session: AsyncSession):
    ...
    @pytest.mark.asyncio
    async def test_admin_scope_can_see_all_posts_regardless_of_owner(self, db_session: AsyncSession):
        blog = await create_blog(db_session, owner_id="auth0|someone-else")
        post = await create_post(db_session, blog=blog)
        policy = make_policy("auth0|admin", "admin")
        query = policy.scope("publish").where(Post.id == post.id)
    ...
    @pytest.mark.asyncio
    async def test_own_scope_can_see_multiple_own_posts(self, db_session: AsyncSession):
        blog = await create_blog(db_session, owner_id="auth0|user1")
    ...
    @pytest.mark.asyncio
    async def test_own_scope_does_not_return_other_users_posts_in_full_list(self, db_session: AsyncSession):
